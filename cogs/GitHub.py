...
class GitHub(commands.Cog):
    ...
    async def repos(self, ctx:discord.ext.commands.Context, username):
        ...
        embed.set_footer(text=f"{username} has {user_data['public_repos']} repositories!")
        if len(repos_data) > 5:
        ...
        for attribute in attributes:
            if user_data[attribute] is not None:
                if user_data[attribute] != "":
                    embed.add_field(name=attributes[attribute], value=user_data[attribute], inline=False)
        await ctx.send(embed=embed)

...
class Currency(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.defined_currencies = json.loads(open('./main_resources/Assets/currency_values.json', encoding='utf-8').read())
    ...
    async def balance(self, ctx:commands.Context, member:discord.Member=None):
        ...
        if member is None:
            ...
            if coins is None:
                ...
                emb = discord.Embed(description=f"***{ctx.author.name} currently has 0 coins. Poor much?***", color=discord.Colour.green())
            else:
                emb = discord.Embed(description=f"***{ctx.author.name} currently has {coins['currency']} coins.***", color=discord.Colour.green())
        else:
            ...
            if coins is None:
                ...
                emb = discord.Embed(description=f"***{member.name} currently has 0 coins. Poor much?***", color=discord.Colour.green())
            else:
                emb = discord.Embed(description=f"***{member.name} currently has {coins['currency']} coins.***", color=discord.Colour.green())
        await ctx.send(embed=emb)
    ...
    async def addmoney(self, ctx:commands.Context, member:discord.Member, amount:int):
        ...
        emb = discord.Embed(description=f"***Added {amount} coins to {member.name}'s balance.***", color=discord.Colour.green())
        await ctx.send(embed=emb)

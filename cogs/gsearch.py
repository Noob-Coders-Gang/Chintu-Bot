...
class Google(commands.Cog):
    ...
    async def gsearch(self, ctx: discord.ext.commands.Context, *, query):
        ...
        searchInput = "https://google.com/search?q=" + parse.quote(query) + "&num=2"  # Query url for top 2 results
        res = requests.get(searchInput)  # Gets the google search results page
        soup = BeautifulSoup(res.text, "html.parser")  # Parses the google search results page
